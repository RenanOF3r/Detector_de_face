...
def transformar_coordenadas_box(box_rotada, angulo_rotacao, dim_originais, dim_rotadas):
    ...
    try:
        ...
        xmax_r = xmin_r + w_r; ymax_r = ymin_r + h_r
        xmin_r = max(0, xmin_r); ymin_r = max(0, ymin_r)
        xmax_r = min(w_rot, xmax_r); ymax_r = min(h_rot, ymax_r)
        ...
        print(f"  ERRO Transformação (Exceção): {e}", flush=True); return None
...
def aplicar_nms(caixas, limiar_iou=0.4):
    ...
    if not caixas_formato_nms: return []
    try:
        indices = cv2.dnn.NMSBoxes(caixas_formato_nms, scores, score_threshold=0.0, nms_threshold=limiar_iou)
    except Exception as e:
        print(f"Erro durante NMS: {e}", flush=True); return caixas_formato_nms # Fallback
...
# --- Função de Processamento Principal (SEM CACHE PARA TESTE) ---
# @st.cache_data(show_spinner=False) # <--- Linha comentada
def detectar_e_desenhar_rostos(imagem_pil, bytes_imagem): # Mantém argumentos por consistência, mas bytes_imagem não é usado pelo cache agora
    """
    ...
    CACHE DESABILITADO PARA TESTE.
    """
    print(f"\n[{time.time()}] ===== INICIANDO DETECÇÃO (CACHE DESABILITADO, NMS Ativo, Bytes: {len(bytes_imagem)}) =====", flush=True)
    try:
        ...
        if h_orig == 0 or w_orig == 0:
            st.error("Erro: Imagem carregada tem dimensões inválidas."); return 0, None
        ...
        for angulo in [0, 90, 180, 270]:
            ...
            if resultados.detections:
                for detection in resultados.detections:
                    ...
                    if box_relativa:
                        if not (0 <= box_relativa.xmin <= 1 and 0 <= box_relativa.ymin <= 1 and box_relativa.width > 0 and box_relativa.height > 0): continue
                        xmin_r = math.floor(box_relativa.xmin * w_rot); ymin_r = math.floor(box_relativa.ymin * h_rot)
                        w_r = math.ceil(box_relativa.width * w_rot); h_r = math.ceil(box_relativa.height * h_rot)
                        box_abs_rotada = (xmin_r, ymin_r, w_r, h_r)
                        box_abs_original = transformar_coordenadas_box(box_abs_rotada, angulo, (h_orig, w_orig), (h_rot, w_rot))
                        if box_abs_original: deteccoes_antes_nms.append(box_abs_original)
        ...
        else: print("  Nenhum rosto detectado após NMS.")
...
arquivo_imagem_enviado = st.file_uploader(
    ...
    # key=f"uploader_{int(time.time())}" # Adicionar uma chave dinâmica PODE forçar re-renderização, mas vamos tentar sem primeiro
)

if arquivo_imagem_enviado is not None:
    try:
        bytes_da_imagem = arquivo_imagem_enviado.getvalue()
        imagem_pil = Image.open(io.BytesIO(bytes_da_imagem))
        ...
        with coluna_processada:
            ...
            with st.spinner('Detectando rostos (cache desabilitado)...'):
                # Chama a função SEM cache
            ...
            if imagem_final_bgr is not None and num_rostos >= 0:
                ...
                if sucesso_encode:
                    ...
                    st.download_button(
                       label="💾 Baixar Imagem Processada", data=img_bytes_download,
                       file_name=f"rostos_detectados_{nome_base}.png", mime="image/png")
                else: st.warning("Não foi possível gerar o arquivo para download.")
            elif num_rostos == 0: st.info("Nenhum rosto detectado na imagem após processamento e NMS.")
            else: st.error("Falha ao processar a imagem. Verifique os logs se disponíveis.")
    except Exception as e:
...
else: st.info("☝️ Faça o upload de uma imagem para iniciar a detecção.")
...
"""
st.sidebar.info(info_sidebar_text)
